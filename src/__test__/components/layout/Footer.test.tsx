...
describe("Footer Component", () => {
	...
	it("should render links with correct hrefs", () => {
		...
		const githubLink = links.find(
			(link) => link.getAttribute("href") === props.githubLink,
		);
		const linkedinLink = links.find(
			(link) => link.getAttribute("href") === props.linkedinLink,
		);
	...
	it("should render images with correct src", () => {
		...
		const githubImg = images.find(
			(img) => img.getAttribute("src") === props.githubIcon,
		);
		const linkedinImg = images.find(
			(img) => img.getAttribute("src") === props.linkedinIcon,
		);
